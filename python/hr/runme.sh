...
#compile
python -m py_compile ./$handsOnNumber/$challengeNumber/$sourceFileName.py

#python exit status is 0 upon successful compilation
if [ `echo $?` -eq 0 ]
then
	...
	do 
		...
		python ./$handsOnNumber/$challengeNumber/$sourceFileName.py<./$handsOnNumber/$challengeNumber/input/$inFileName>./$handsOnNumber/$challengeNumber/demo/$outFileName;
	done;
